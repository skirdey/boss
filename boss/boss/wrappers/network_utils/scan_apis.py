...
class APIScanner:
    ...
    def scan_api(self, ports: Optional[List[int]] = None) -> List[Dict]:
        """
        Scan for common API endpoints across specified ports.

        Args:
            ports (Optional[List[int]]): List of ports to scan. Defaults to self.ports.

        Returns:
            List[Dict]: List of discovered API endpoints.
        """
        api_endpoints_found = []
        ...
        def scan_port_api(port):
            ...
            for path in api_paths:
                url = f"{protocol}://{self._get_base_domain()}:{port}{path}"
                try:
                    request = urllib.request.Request(
                        url, headers={"User-Agent": "WESSyScanner"}
                    )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            executor.map(scan_port_api, ports_to_scan)
    ...
    def scan(
        ...
        """Perform all scanning tasks and compile results."""
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            if scan_api_enabled:
                api_scan_future = executor.submit(self.scan_api, api_ports)
                self.results["api_endpoints"] = api_scan_future.result()
