    ...
    def process_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            previous_step_results = "\n".join(
                [serialize_task_to_string(step) for step in previous_steps if step]
            )
            ...
            if not ports_to_scan:
                ...
                return self._create_task_result(
                    task.get("_id"), error="Failed to determine ports to scan"
                )
            ...
            return self._create_task_result(
                task.get("_id", "unknown"), error=f"Validation error: {ve}"
            )
        except Exception as e:
            logger.error(f"Error processing task: {str(e)}")
            return self._create_task_result(task.get("_id", "unknown"), error=str(e))
