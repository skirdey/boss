...
def expected_certificate():
    ...
    not_before = datetime(2023, 1, 1, 0, 0)
    not_after = datetime(2023, 12, 31, 23, 59, 59)
    return {
        "subject": {"commonName": "www.example.com"},
        "issuer": {"commonName": "ExampleCA"},
        "version": 3,
...
def test_connection_timeout():
    ...
    with patch(
        ...
        result = get_ssl_certificate("example.com")
        assert result == "Connection error: Connection timed out"
...
def test_ssl_error():
    ...
    with patch("socket.create_connection") as mock_socket:
        with patch("ssl.SSLContext.wrap_socket", side_effect=ssl.SSLError("SSL error")):
            result = get_ssl_certificate("example.com")
            assert result == "SSL error: ('SSL error',)"


def test_invalid_certificate_format(mock_ssl_certificate):
    """Test handling of invalid certificate format."""
    invalid_cert = mock_ssl_certificate.copy()
    invalid_cert["notBefore"] = "Invalid Date Format"
    ...
    with patch("socket.create_connection") as mock_socket:
        with patch("ssl.SSLContext.wrap_socket") as mock_wrap_socket:
            ...
            assert result["notBefore"] == "Invalid Date Format"
...
def test_missing_certificate_fields():
    ...
    with patch("socket.create_connection") as mock_socket:
        with patch("ssl.SSLContext.wrap_socket") as mock_wrap_socket:
            ...
            result = get_ssl_certificate("example.com")
            assert result["notBefore"] == "Unknown"
            assert result["notAfter"] == "Unknown"


def test_no_certificate():
    """Test handling of no certificate returned."""
    with patch("socket.create_connection") as mock_socket:
        with patch("ssl.SSLContext.wrap_socket") as mock_wrap_socket:
            mock_ssl_socket = MagicMock()
            ...
            result = get_ssl_certificate("example.com")
            assert result == "No certificate found"
